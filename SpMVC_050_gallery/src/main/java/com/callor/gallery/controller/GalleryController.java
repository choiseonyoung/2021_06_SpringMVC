...
public class GalleryController {
	 ...
	 * 
	 * a tag를 클릭했을 때 : <a href="${rootPath}/gallery/dumy> 열기</a>
	 * JS : location.href="${rootPath}/gallery/dumy" 가 실행됐을 때
	 */
...
//			List<GalleryDTO> gallerPageList = gaService.selectAllPage(intPageNum,model);
	...
	public String input(Model model, HttpSession session) {
		...
		GalleryDTO gaDTO = GalleryDTO.builder().g_date(curDate).g_time(curTime).g_writer("choiseonyoung").build();
	...
	public String delete(@RequestParam("g_seq") String seq, HttpSession session) {
		...
		// 로그인 했을 때 세션에 부착한 데이터
		//if(memVO == null) { // 로그인이 안 되었다면
		//	return "redirect:/member/login";
		//}
	...
	public String file_delete(@PathVariable("seq") String seq) {
		...
		if(ret > 0) return "OK"; // 정상적으로 파일 삭제됐
		else return "FAIL";
	}
}
