...
public class StudentServiceImplV1 implements StudentService {
	...
	public String makeStNum(String prefix) {
		 ...
		 * stSeq = stNum.substring(4) 이런 형식의 코드 생성 stSeq에는 0010의 문자열만 남게 된다
		 */
...
//		stVO.setSt_num(newStNum);
	...
	public String detail(Model model, String st_num) {
		...
		// 중간에 데이터 잘못 전해졌을 때 컨트롤러에 알리기 위함
	...
	/*
	 * Transaction의 조건 다수의 CRUD는 한 개의 업무 프로세스다 다수의 CRUD가 모두 정상적으로 완료되어야만 업무가 정상으로
	 * 수행된다.
	 * 
	 * 업무가 수행되는 동안 한 곳이라도 CRUD에서 오류가 발생하면 그 중 CUD가 진행되는 동안문제가 발생하고 데이터에 오류가 저장될 것이다
	 * 
	 * 이런 상황을 방지하기 위하여 업무 단위를 Transaction이라는 단위로 묶고
	 * 
	 * 모든 업무가 완료되면 데이터를 Commit(실제저장)하고 그렇지 않으면 Rollback ALL(모두 취소)하는 처리
	 */
	@Transactional
	@Override
	public String scoreInput(ScoreInputVO scInputVO) {
		...
		*/
		...
		/*
		 * @Transactional로 선언된 method에서 모든 데이터를 insertOrUpdate를 수행한 다음 강제로 exception을
		 * 발생하였다
		 * 
