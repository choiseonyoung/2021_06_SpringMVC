...
<script>
	...
	const student_submit = () => {
		...
		if(st_num.value === "") {
			alert("학번은 반드시 입력하세요")
			st_num.focus()
			// event 핸들링 코드에서 코드 진행을 멈추고자 할 때는
	...
	document.querySelector("form#student_input").addEventListener("click",(e)=> {
		...
		if(target.tagName === "BUTTON") {
			if(target.className.includes("save")) {
				/*
				tag에 class를 지정할 때 1개만 지정할 수도 있지만 다수의 class를 지정하는 경우도 있다
				이 때는 if(target.className === "className") 형식으로 class를 검사할 수 없다
				이럴 때는 includes를 사용하여 포함된 클래스를 검사해야 한다.
				*/
				if(target.className.includes("save")) {
