 ...
 * 
 * 내부에서
 * 요청정보를 OpenAPI로 보내고 수신된 데이터를 DTO(VO) 구조에 따라 자체 parsing을 하고
 * 결과를 돌려준다.
 */
...
public class NaverNewsServiceImplV2 extends NaverNewsServiceImplV1 {
	...
	@Override
	public List<NewsDTO> getNaverList(String queryURL) throws ParseException, URISyntaxException {
		
		/*
		 * json-simple, Gson 등으로 parsing을 수행할 때는
		 * URL 클래스를 사용하여 naver에 요청하고
		 * 결과는 jsonString으로 받았다
		 * 
		 * URI 클래스를 사용하여 naver에 요청하는 정보를 생성
		...
		// 네이버한테 내가 필요한 데이터는 JSON 타입이니 JSON 타입으로 달라
