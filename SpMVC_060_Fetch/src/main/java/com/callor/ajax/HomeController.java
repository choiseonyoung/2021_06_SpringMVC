...
public class HomeController {
	 ...
	 * Web에서 JSON 데이터를 보내면 Controller에서는 @RequestBody 속성을 사용하여 데이터를 받고
	 * Web에 다시 JSON 데이터를 return하기 위해서는 @ResponseBody 속성을 사용하여 데이터를 return한다
	 */
	/*
	 * Controller에서 web에서 보낸 데이터가 2가지 이상의 변수에 해당하면
	 * 무조건 VO, DTO를 만들어서 받아라
	 * @RequestBody는 Jackson Bind에 의해서 Web에서 전달된 JSON 데이터를 DTO 클래스에 binding한다
	 * 
	 * Spring 4.x에서는 Jackson bind가 기본으로 포함이 돼있었다
	 * Spring 5.x에서는 Jackson bind가 분리돼서 pom.xml에서 jackson-bind를 포함시켜줘야 한다.
	 */
	@ResponseBody
